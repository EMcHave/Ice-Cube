...
Ice::Cell::Cell(int i, int j, int k, float r, DirectX::XMFLOAT3 sdvig)
{
	...
	nodes[2] = Ice::Node(-sdvig.x + i * r, -sdvig.y + (j + 1) * r, -sdvig.z + k * r);
	nodes[3] = Ice::Node(-sdvig.x + (i + 1) * r, -sdvig.y + (j + 1) * r, -sdvig.z + k * r);
	nodes[4] = Ice::Node(-sdvig.x + i * r, -sdvig.y + j * r, -sdvig.z + (k + 1) * r);
	nodes[5] = Ice::Node(-sdvig.x + (i + 1) * r, -sdvig.y + j * r, -sdvig.z + (k + 1) * r);
